    ...
    const getPageData = async () => {
        ...
        try {
            const response = await fetch(
                'https://backend.loewenmut.ch/api/impressum?populate[Info_Bereich][populate]=icon'
            );
                ...
                <div className='container'>
                    <div className='banner_content' data-aos='fade-up'>
                        ...
                        <div className='wat_address'>
                            <h2>{pageData?.ueberschrift}</h2>
                            <p><img src='./images/krimi-icon.svg' width='45px' />{pageData?.Text}</p>
                        </div>
                    </div>
                </div>
            </section>
                    ...
                    <div className='container' data-aos='fade-up'>
                        <div className="row">
                            {pageData?.Info_Bereich?.map((item, index) => {
                                ...
                                const iconUrl = item?.icon?.url || item?.icon?.data?.attributes?.url;

                                let link = null;
                                if (title?.toLowerCase().includes('mail')) {
                                    link = `mailto:${desc}`;
                                } else if (title?.toLowerCase().includes('telefon') || title?.toLowerCase().includes('phone')) {
                                    link = `tel:${desc.replace(/\s+/g, '')}`;
                                }
                                ...
                                const Content = (
                                    <div className="item_inner text-center">
                                        {iconUrl && (
                                            <ColoredSVG
                                                url={`https://backend.loewenmut.ch${iconUrl}`}
                                                color="inherit"
                                            />
                                            // <img
                                            //     src={iconUrl.startsWith('http') ? iconUrl : `https://backend.loewenmut.ch${iconUrl}`}
                                            //     alt={title}
                                ...
                                return (
                                    <div key={index} className="col-md-4 imp_detail">
                                        {link ? <a href={link}>{Content}</a> : <address>{Content}</address>}
                                    </div>
                                );
                            })}
                        </div>
