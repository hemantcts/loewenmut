...
const Kontakt = () => {
    ...
    const getPageData = async () => {
        ...
        try {
            const response = await fetch(
                'https://backend.loewenmut.ch/api/kontakt?[populate][Info_Bereich][populate]=*'
            );
    ...
    function formatPhoneNumber(num) {
        ...
        // Format as 3 3 2 2 pattern -> 052 224 77 88
                        ...
                        <div className='col-lg-5 addres_col' data-aos={!isMobile ? 'fade-right' : undefined}>
                            ...
                            {pageData?.Info_Bereich && (
                                pageData?.Info_Bereich.map((item, index) => (
                                    <div className='addres_info' key={index}>
                                        {item?.__component === 'kontakt-components.adresse' &&
                                            <div className='info_item'>
                                                <address>
                                                    <div>
                                                        ...
                                                        <h3>{item?.Titel}</h3>
                                                        <p dangerouslySetInnerHTML={{ __html: item?.Beschreibung?.replace(/\n/g, '<br/>') }} />
                                                    </div>
                                                </address>
                                            </div>
                                        }
                                        {item?.__component === 'kontakt-components.telefon' &&
                                            <div className='info_item'>
                                                <Link to={`tel: ${item?.Telefon_Nummer}`}>
                                                    <ColoredSVG
                                                        url={`https://backend.loewenmut.ch${item?.icon?.url}`}
                                                        color="inherit"
                                                    />
                                                    ...
                                                    <h3>{item?.Titel}</h3>
                                                    <p>{formatPhoneNumber(item?.Telefon_Nummer)}</p>
                                                </Link>
                                            </div>
                                        }
                                        {item?.__component === 'kontakt-components.e-mail' &&
                                            <div className='info_item'>
