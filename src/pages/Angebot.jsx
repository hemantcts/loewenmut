import React from 'react'
import { Link } from 'react-router-dom'

const Angebot = () => {
    return (
        <div className='page_content angebot_page'>
            <section className='wi_full inner_banner'>
                <div className='container'>
                    <div className='banner_content' data-aos='fade-up'>
                        <h1>Wir ticken wie Leistungssportler</h1>
                        <p>Nibh vel velit auctor aliquet. Aenean sollicitudin, lorem quis bibendum auctor, nisi elit consequat ipsum, nec sagittis sem nibh id elit.Nibh vel velit auctor aliquet. Aenean sollicitudin, lorem quis bibendum auctor, nisi elit consequat ipsum, nec sagittis sem nibh id elit.</p>
                    </div>
                </div>
            </section>
            <section className='common_angebot_sec'>
                <div className='wi_full py_3 common_angbot_count' id='branding'>
                    <div className='container'>
                        <div className='sec_heading'>
                            <div className='heading_left' data-aos='fade-up'>
                                <h2>Branding</h2>
                                <p>Nibh vel velit auctor aliquet. Aenean sollicitudin, lorem quis bibendum auctor, nisi elit consequat ipsum, nec sagittis sem nibh id elit.Nibh vel velit auctor aliquet. Aenean sollicitudin, lorem quis bibendum auctor, nisi elit consequat ipsum, nec sagittis sem nibh id elit.</p>
                            </div>
                            <div className='icon_right'>
                                <svg width="244" height="234" viewBox="0 0 244 234" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M122.194 38.9333C104.764 38.9333 88.0398 45.8554 75.7118 58.1859C63.3815 70.5162 56.4592 87.2402 56.4592 104.668C56.4592 122.095 63.3812 138.821 75.7118 151.149C88.0421 163.48 104.766 170.402 122.194 170.402C139.621 170.402 156.347 163.48 168.675 151.149C181.006 138.819 187.928 122.095 187.928 104.668C187.908 87.2379 180.976 70.5343 168.655 58.2037C156.325 45.8834 139.622 38.9512 122.192 38.9313L122.194 38.9333ZM166.455 100.685L152.909 113.912C152.311 114.51 152.033 115.346 152.152 116.183L155.379 134.828C156.046 138.772 154.423 142.756 151.186 145.107C147.949 147.457 143.666 147.766 140.121 145.903L123.348 137.099C122.611 136.701 121.735 136.701 120.998 137.099L104.225 145.903C102.721 146.72 101.038 147.138 99.3248 147.138C96.2274 147.119 93.289 145.744 91.2972 143.374C89.3052 141.013 88.4486 137.886 88.9666 134.828L92.1935 116.143C92.323 115.326 92.0541 114.5 91.4764 113.912L78.0107 100.685C75.1522 97.8963 74.1264 93.7232 75.3613 89.9184C76.5863 86.1137 79.873 83.345 83.8272 82.7572L102.592 80.0481C103.418 79.9087 104.126 79.3908 104.504 78.6537L112.87 61.6822C114.643 58.1066 118.298 55.8356 122.292 55.8356C126.286 55.8356 129.942 58.1064 131.714 61.6822L140.081 78.6537C140.469 79.4007 141.196 79.9286 142.033 80.0481L160.757 82.7572C164.722 83.3349 168.008 86.1137 169.243 89.9284C170.468 93.753 169.383 97.9364 166.455 100.685ZM186.095 182.277C196.245 179.369 203.804 170.863 205.497 160.445C206.244 155.873 208.635 151.73 212.23 148.812C220.407 142.059 223.933 131.183 221.274 120.923C220.089 116.441 220.587 111.68 222.668 107.537C227.37 98.0847 226.174 86.7603 219.601 78.4925C216.792 74.8472 215.378 70.3056 215.617 65.7041C216.065 55.1466 210.368 45.2862 200.996 40.4058C196.882 38.2744 193.675 34.7186 191.992 30.406C188.137 20.5953 178.934 13.9122 168.407 13.2748C163.775 13.0058 159.393 11.0437 156.096 7.77701C148.557 0.386842 137.422 -1.97371 127.532 1.72138C123.209 3.35479 118.428 3.35479 114.106 1.72138C104.216 -1.97371 93.0803 0.38674 85.541 7.77701C82.2442 11.0437 77.8619 13.006 73.2306 13.2748C62.703 13.9122 53.4999 20.5953 49.6455 30.406C47.9622 34.7186 44.7551 38.2742 40.6418 40.4058C31.2695 45.2862 25.5724 55.1463 26.0206 65.7041C26.2796 70.3056 24.8653 74.8472 22.0366 78.4925C15.4631 86.7593 14.2678 98.0837 18.969 107.537C21.0507 111.68 21.5586 116.431 20.4033 120.923C17.7041 131.181 21.22 142.068 29.4069 148.812C32.9128 151.77 35.2336 155.903 35.9406 160.445C37.6737 170.863 45.2931 179.329 55.4619 182.158C59.9339 183.403 63.7983 186.241 66.3381 190.126C72.1446 198.97 82.5629 203.631 93.0313 202.077C97.603 201.4 102.274 202.396 106.178 204.866C115.083 210.554 126.477 210.554 135.381 204.866C139.285 202.396 143.956 201.4 148.527 202.077C158.986 203.611 169.394 198.95 175.221 190.126C177.78 186.281 181.643 183.492 186.095 182.277ZM122.352 178.293C102.791 178.333 84.0165 170.604 70.1613 156.79C56.307 142.985 48.5083 124.23 48.4885 104.67C48.4686 85.1114 56.2274 66.3453 70.0516 52.5002C83.8761 38.6658 102.63 30.8871 122.191 30.8871C141.753 30.8871 160.506 38.6658 174.331 52.5002C188.155 66.3446 195.914 85.1089 195.894 104.67C195.874 124.212 188.105 142.947 174.281 156.762C160.466 170.586 141.731 178.355 122.19 178.375L122.352 178.293ZM161.674 92.3976C161.963 93.3139 161.724 94.3099 161.037 94.9871L147.332 108.214C144.862 110.624 143.736 114.09 144.304 117.496L147.531 136.181C147.69 137.137 147.292 138.093 146.505 138.661C145.718 139.229 144.682 139.299 143.826 138.85L127.053 130.046C124.006 128.413 120.34 128.413 117.293 130.046L100.52 138.85C99.6736 139.309 98.6278 139.249 97.8409 138.671C97.064 138.103 96.6756 137.137 96.8548 136.181L100.042 117.496C100.63 114.09 99.4942 110.614 97.0142 108.214L83.4687 94.9871C82.7815 94.3098 82.5325 93.3038 82.8213 92.3876C83.1201 91.4613 83.9069 90.794 84.8631 90.6446L103.628 87.9355C107.064 87.4376 110.032 85.2961 111.596 82.1986L119.962 65.187C120.4 64.3105 121.296 63.7727 122.272 63.7926C123.239 63.7627 124.135 64.3105 124.543 65.187L132.91 82.1986C134.473 85.2861 137.451 87.4276 140.878 87.9355L159.602 90.6446C160.578 90.7642 161.395 91.4514 161.674 92.3976ZM69.3657 204.267L41.1197 232.513C40.3827 233.26 39.3867 233.689 38.3308 233.709C37.9623 233.748 37.5838 233.748 37.2153 233.709C35.8209 233.31 34.7452 232.175 34.4264 230.761L28.6497 205.263L3.11149 199.287C1.69717 198.968 0.561744 197.893 0.163474 196.498C-0.254836 195.084 0.143556 193.55 1.19931 192.514L28.7671 164.946C31.9244 177.038 41.2968 186.54 53.3481 189.846C55.9476 190.573 58.1985 192.217 59.6827 194.468C62.2026 198.372 65.4911 201.707 69.3657 204.267ZM243.505 196.299C243.107 197.694 241.972 198.77 240.557 199.088L215.019 205.064L209.242 230.403C208.924 231.817 207.848 232.952 206.454 233.351C206.085 233.39 205.707 233.39 205.338 233.351C204.282 233.331 203.286 232.902 202.549 232.155L173.625 203.392C176.952 200.981 179.82 197.983 182.071 194.547C183.555 192.296 185.806 190.653 188.405 189.926C201.054 186.43 210.696 176.161 213.385 163.312L242.666 192.593C243.622 193.649 243.941 195.143 243.503 196.497L243.505 196.299Z" fill="inherit" />
                                </svg>
                            </div>
                        </div>
                        <div className='sec_body'>
                            <div className='row'>
                                <div className='col-md-6 col_item' data-aos='fade-right'>
                                    <div className='col_inner'>
                                        <h3>Beratung Strategie</h3>
                                        <p>Wenn Funktionalität gefragt ist, konzipieren und entwickeln wir Ihre Webapplikation. Von der Idee bis zum Go-Live. Gemeinsam schaffen wir Innovation.</p>
                                        <div className='btn_block'>
                                            <Link to={'/'} className='button border_btn'>
                                                <span>Mehr erfahren
                                                <svg width="40" height="15" viewBox="0 0 40 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <path d="M39.9424 7.06055L32.4659 14.1211L30.9535 12.5198L36.7333 7.06055L30.9535 1.60128L32.4659 0L39.9424 7.06055Z" fill="inherit" />
                                                    <path d="M38.3375 5.95908V8.16201H0V5.95908H38.3375Z" fill="inherit" />
                                                </svg></span>
                                            </Link>
                                        </div>
                                    </div>
                                </div>
                                <div className='col-md-6 col_item' data-aos='fade-left'>
                                    <div className='col_inner'>
                                        <h3>Konzept</h3>
                                        <p>Begeistern Sie Ihre Kunden mit von uns konzipierten, designten und realisierten Mobile Apps. Profitieren Sie von den neuen digitalen Geschäftsmodellen...</p>
                                        <div className='btn_block'>
                                            <Link to={'/'} className='button border_btn'>
                                                <span>Mehr erfahren
                                                    <svg width="40" height="15" viewBox="0 0 40 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        <path d="M39.9424 7.06055L32.4659 14.1211L30.9535 12.5198L36.7333 7.06055L30.9535 1.60128L32.4659 0L39.9424 7.06055Z" fill="inherit" />
                                                        <path d="M38.3375 5.95908V8.16201H0V5.95908H38.3375Z" fill="inherit" />
                                                    </svg></span>
                                            </Link>
                                        </div>
                                    </div>
                                </div>
                                <div className='col-md-6 col_item' data-aos='fade-right'>
                                    <div className='col_inner'>
                                        <h3>Logodesign</h3>
                                        <p>UX (Nutzererlebnis) steht bei uns im Fokus und ist fixer Bestandteil jeder Unternehmensstrategie. Ein positives Kundenerlebnis führt zu Zufriedenheit und Reputation.</p>
                                        <div className='btn_block'>
                                            <Link to={'/'} className='button border_btn'>
                                                <span>Mehr erfahren
                                                    <svg width="40" height="15" viewBox="0 0 40 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        <path d="M39.9424 7.06055L32.4659 14.1211L30.9535 12.5198L36.7333 7.06055L30.9535 1.60128L32.4659 0L39.9424 7.06055Z" fill="inherit" />
                                                        <path d="M38.3375 5.95908V8.16201H0V5.95908H38.3375Z" fill="inherit" />
                                                    </svg></span>
                                            </Link>
                                        </div>
                                    </div>
                                </div>
                                <div className='col-md-6 col_item' data-aos='fade-left'>
                                    <div className='col_inner'>
                                        <h3>Corporate Design</h3>
                                        <p>Logodesign, CI/CD, Flyer, Broschüren... Wir schaffen kreative und einzigartige visuelle Identitäten und Produkte.</p>
                                        <div className='btn_block'>
                                            <Link to={'/'} className='button border_btn'>
                                                <span>Mehr erfahren
                                                    <svg width="40" height="15" viewBox="0 0 40 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        <path d="M39.9424 7.06055L32.4659 14.1211L30.9535 12.5198L36.7333 7.06055L30.9535 1.60128L32.4659 0L39.9424 7.06055Z" fill="inherit" />
                                                        <path d="M38.3375 5.95908V8.16201H0V5.95908H38.3375Z" fill="inherit" />
                                                    </svg></span>
                                            </Link>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div className='wi_full py_3 common_angbot_count' id='online_web'>
                    <div className='container'>
                        <div className='sec_heading'>
                            <div className='heading_left' data-aos='fade-up'>
                                <h2>Online / Website</h2>
                                <p>Nibh vel velit auctor aliquet. Aenean sollicitudin, lorem quis bibendum auctor, nisi elit consequat ipsum, nec sagittis sem nibh id elit.Nibh vel velit auctor aliquet. Aenean sollicitudin, lorem quis bibendum auctor, nisi elit consequat ipsum, nec sagittis sem nibh id elit.</p>
                            </div>
                            <div className='icon_right'>
                                <svg width="207" height="213" viewBox="0 0 207 213" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M116.909 93.4282C118.99 93.4282 120.984 93.8655 122.812 94.5563L129.668 97.0748V95.5969C129.668 87.5254 129.318 79.8914 128.627 72.5991H62.5773C61.8865 79.8922 61.4492 87.5266 61.4492 95.5969C61.4492 103.667 61.8865 111.303 62.5773 118.595H101.544L100.59 113.47C99.6372 108.433 100.853 103.405 104.062 99.5836C107.271 95.6746 111.959 93.4185 116.908 93.4185L116.909 93.4282Z" fill="inherit" />
                                    <path d="M184.716 61.3011C173.628 32.5835 149.09 10.5819 118.832 2.97266C128.277 15.6963 135.36 36.1587 138.744 61.3011H184.716Z" fill="inherit" />
                                    <path d="M50.194 95.5891C50.194 87.6313 50.5438 79.9533 51.1822 72.5823L2.91206 72.591C1.07566 79.9804 0 87.6584 0 95.5978C0 103.537 1.08437 111.225 2.91206 118.605H51.1822C50.5438 111.233 50.194 103.555 50.194 95.5978V95.5891Z" fill="inherit" />
                                    <path d="M127.289 61.3014C121.876 23.9963 108.191 0 95.5988 0C82.9975 0 69.3213 23.9963 63.9082 61.3014H127.289Z" fill="inherit" />
                                    <path d="M6.46191 129.879C17.5503 158.597 42.0881 180.598 72.3459 188.207C62.9014 175.484 55.8182 155.021 52.4341 129.879H6.46191Z" fill="inherit" />
                                    <path d="M72.3547 2.97339C42.0879 10.5725 17.5588 32.5842 6.4707 61.3019H52.4417C55.8259 36.1616 62.9191 15.698 72.3547 2.97339Z" fill="inherit" />
                                    <path d="M140.948 95.5978C140.948 97.5041 140.948 99.3319 140.86 101.151L188.161 118.597H188.249C190.068 111.216 191.196 103.494 191.196 95.5989C191.196 87.6149 190.067 79.9806 188.249 72.6011H139.994C140.606 79.9817 140.948 87.6147 140.948 95.5978Z" fill="inherit" />
                                    <path d="M63.8809 129.879C69.3463 167.202 82.9706 191.153 95.5625 191.153C101.29 191.153 107.281 186.204 112.571 177.267L103.634 129.878L63.8809 129.879Z" fill="inherit" />
                                    <path d="M205.679 187.717L178.168 155.388L199.925 144.877C204.288 142.656 204.026 136.281 199.391 134.698L118.956 105.096C114.907 103.566 110.85 107.002 111.68 111.252L127.482 195.684C128.462 200.537 134.714 201.858 137.434 197.905L151.408 178.194L178.516 210.691C180.501 213.052 184.06 213.34 186.412 211.346L205.021 195.606C207.374 193.612 207.671 190.062 205.686 187.71L205.679 187.717Z" fill="inherit" />
                                </svg>
                            </div>
                        </div>
                        <div className='sec_body'>
                            <div className='row'>
                                <div className='col-md-6 col_item' data-aos='fade-right'>
                                    <div className='col_inner'>
                                        <h3>Individuelle Website</h3>
                                        <p>Wir lassen von Ihnen hören. Mit unseren eigens individuell für sie erarbeiteten Kampagnen erreichen Sie die definierten Ziele.</p>
                                        <div className='btn_block'>
                                            <Link to={'/'} className='button border_btn'>
                                                <span>Mehr erfahren
                                                    <svg width="40" height="15" viewBox="0 0 40 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        <path d="M39.9424 7.06055L32.4659 14.1211L30.9535 12.5198L36.7333 7.06055L30.9535 1.60128L32.4659 0L39.9424 7.06055Z" fill="inherit" />
                                                        <path d="M38.3375 5.95908V8.16201H0V5.95908H38.3375Z" fill="inherit" />
                                                    </svg></span>
                                            </Link>
                                        </div>
                                    </div>
                                </div>
                                <div className='col-md-6 col_item' data-aos='fade-left'>
                                    <div className='col_inner'>
                                        <h3>Online Marketing</h3>
                                        <p>Wir leiten Schulungen für Unternehmen, Teams und Einzelpersonen zu diversen digitalen Themen. Wir freuen uns, Sie in die digitale Welt mitzunehmen.</p>
                                        <div className='btn_block'>
                                            <Link to={'/'} className='button border_btn'>
                                                <span>Mehr erfahren
                                                    <svg width="40" height="15" viewBox="0 0 40 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        <path d="M39.9424 7.06055L32.4659 14.1211L30.9535 12.5198L36.7333 7.06055L30.9535 1.60128L32.4659 0L39.9424 7.06055Z" fill="inherit" />
                                                        <path d="M38.3375 5.95908V8.16201H0V5.95908H38.3375Z" fill="inherit" />
                                                    </svg></span>
                                            </Link>
                                        </div>
                                    </div>
                                </div>
                                <div className='col-md-6 col_item' data-aos='fade-right'>
                                    <div className='col_inner'>
                                        <h3>Online-Shop</h3>
                                        <p>Wir haben höchste Ansprüche und es soll kinderleicht zu bedienen sein. Wir geben Ihrer Webseite ein performantes Zuhause.</p>
                                        <div className='btn_block'>
                                            <Link to={'/'} className='button border_btn'>
                                                <span>Mehr erfahren
                                                    <svg width="40" height="15" viewBox="0 0 40 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        <path d="M39.9424 7.06055L32.4659 14.1211L30.9535 12.5198L36.7333 7.06055L30.9535 1.60128L32.4659 0L39.9424 7.06055Z" fill="inherit" />
                                                        <path d="M38.3375 5.95908V8.16201H0V5.95908H38.3375Z" fill="inherit" />
                                                    </svg></span>
                                            </Link>
                                        </div>
                                    </div>
                                </div>
                                <div className='col-md-6 col_item' data-aos='fade-left'>
                                    <div className='col_inner'>
                                        <h3>Motion Design</h3>
                                        <p>Die richtige Strategie ist die Grundlage für Ihren Erfolg. Wir stellen die richtigen Fragen und entwickeln Ihre individuelle Roadmap. Der Weg ist das Ziel.</p>
                                        <div className='btn_block'>
                                            <Link to={'/'} className='button border_btn'>
                                                <span>Mehr erfahren
                                                    <svg width="40" height="15" viewBox="0 0 40 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        <path d="M39.9424 7.06055L32.4659 14.1211L30.9535 12.5198L36.7333 7.06055L30.9535 1.60128L32.4659 0L39.9424 7.06055Z" fill="inherit" />
                                                        <path d="M38.3375 5.95908V8.16201H0V5.95908H38.3375Z" fill="inherit" />
                                                    </svg></span>
                                            </Link>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div className='wi_full py_3 common_angbot_count' id='print'>
                    <div className='container'>
                        <div className='sec_heading'>
                            <div className='heading_left' data-aos='fade-up'>
                                <h2>Print</h2>
                                <p>Nibh vel velit auctor aliquet. Aenean sollicitudin, lorem quis bibendum auctor, nisi elit consequat ipsum, nec sagittis sem nibh id elit.Nibh vel velit auctor aliquet. Aenean sollicitudin, lorem quis bibendum auctor, nisi elit consequat ipsum, nec sagittis sem nibh id elit.</p>
                            </div>
                            <div className='icon_right'>
                                <svg width="202" height="187" viewBox="0 0 202 187" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M142.648 4.85999C142.648 2.1759 140.472 0 137.788 0H63.5973C60.9132 0 58.7373 2.1759 58.7373 4.85999V95.8976H142.648V4.85999Z" fill="inherit" />
                                    <path d="M172.879 47.9485H147.443V100.692H53.9424V47.9485H28.5056C12.7544 47.9485 0 60.7029 0 76.4541V158.494C0 174.246 12.7544 187 28.5056 187H172.879C188.63 187 201.385 174.246 201.385 158.494V76.4541C201.385 60.7029 188.63 47.9485 172.879 47.9485ZM24.5737 69.5254C21.9257 69.5254 19.7789 67.3787 19.7789 64.7306C19.7789 62.0823 21.9257 59.9357 24.5737 59.9357C27.2218 59.9357 29.3686 62.0823 29.3686 64.7306C29.3686 67.3787 27.2218 69.5254 24.5737 69.5254Z" fill="inherit" />
                                </svg>
                            </div>
                        </div>
                        <div className='sec_body'>
                            <div className='row'>
                                <div className='col-md-6 col_item' data-aos='fade-right'>
                                    <div className='col_inner'>
                                        <h3>Drucksachen</h3>
                                        <p>Wenn Funktionalität gefragt ist, konzipieren und entwickeln wir Ihre Webapplikation. Von der Idee bis zum Go-Live. Gemeinsam schaffen wir Innovation.</p>
                                        <div className='btn_block'>
                                            <Link to={'/'} className='button border_btn'>
                                                <span>Mehr erfahren
                                                    <svg width="40" height="15" viewBox="0 0 40 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        <path d="M39.9424 7.06055L32.4659 14.1211L30.9535 12.5198L36.7333 7.06055L30.9535 1.60128L32.4659 0L39.9424 7.06055Z" fill="inherit" />
                                                        <path d="M38.3375 5.95908V8.16201H0V5.95908H38.3375Z" fill="inherit" />
                                                    </svg></span>
                                            </Link>
                                        </div>
                                    </div>
                                </div>
                                <div className='col-md-6 col_item' data-aos='fade-left'>
                                    <div className='col_inner'>
                                        <h3>Beschriftungen</h3>
                                        <p>Begeistern Sie Ihre Kunden mit von uns konzipierten, designten und realisierten Mobile Apps. Profitieren Sie von den neuen digitalen Geschäftsmodellen...</p>
                                        <div className='btn_block'>
                                            <Link to={'/'} className='button border_btn'>
                                                <span>Mehr erfahren
                                                    <svg width="40" height="15" viewBox="0 0 40 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        <path d="M39.9424 7.06055L32.4659 14.1211L30.9535 12.5198L36.7333 7.06055L30.9535 1.60128L32.4659 0L39.9424 7.06055Z" fill="inherit" />
                                                        <path d="M38.3375 5.95908V8.16201H0V5.95908H38.3375Z" fill="inherit" />
                                                    </svg></span>
                                            </Link>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div className='wi_full py_3 common_angbot_count' id='content'>
                    <div className='container'>
                        <div className='sec_heading'>
                            <div className='heading_left' data-aos='fade-up'>
                                <h2>Content</h2>
                                <p>Nibh vel velit auctor aliquet. Aenean sollicitudin, lorem quis bibendum auctor, nisi elit consequat ipsum, nec sagittis sem nibh id elit.Nibh vel velit auctor aliquet. Aenean sollicitudin, lorem quis bibendum auctor, nisi elit consequat ipsum, nec sagittis sem nibh id elit.</p>
                            </div>
                            <div className='icon_right'>
                                <svg width="213" height="178" viewBox="0 0 213 178" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M212.322 49.1394C207.488 14.6706 175.022 -5.9047 144.63 1.50684C122.282 6.95807 109.053 28.0237 106.499 39.4155C103.945 28.0227 90.7159 6.96552 68.3688 1.50684C37.9679 -5.90555 5.50949 14.6696 0.676617 49.1394C-2.64314 72.8057 6.76702 92.6748 17.6831 109.427C37.7179 140.165 72.9874 164.337 95.2945 175.382C102.35 178.873 110.653 178.873 117.708 175.382C140.007 164.339 175.283 140.165 195.32 109.427C206.236 92.6748 215.637 72.7972 212.326 49.1394H212.322Z" fill="inherit" />
                                </svg>
                            </div>
                        </div>
                        <div className='sec_body'>
                            <div className='row'>
                                <div className='col-md-6 col_item' data-aos='fade-right'>
                                    <div className='col_inner'>
                                        <h3>Text</h3>
                                        <p>Wir lassen von Ihnen hören. Mit unseren eigens individuell für sie erarbeiteten Kampagnen erreichen Sie die definierten Ziele.</p>
                                        <div className='btn_block'>
                                            <Link to={'/'} className='button border_btn'>
                                                <span>Mehr erfahren
                                                    <svg width="40" height="15" viewBox="0 0 40 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        <path d="M39.9424 7.06055L32.4659 14.1211L30.9535 12.5198L36.7333 7.06055L30.9535 1.60128L32.4659 0L39.9424 7.06055Z" fill="inherit" />
                                                        <path d="M38.3375 5.95908V8.16201H0V5.95908H38.3375Z" fill="inherit" />
                                                    </svg></span>
                                            </Link>
                                        </div>
                                    </div>
                                </div>
                                <div className='col-md-6 col_item' data-aos='fade-left'>
                                    <div className='col_inner'>
                                        <h3>Fotografie</h3>
                                        <p>Wir leiten Schulungen für Unternehmen, Teams und Einzelpersonen zu diversen digitalen Themen. Wir freuen uns, Sie in die digitale Welt mitzunehmen.</p>
                                        <div className='btn_block'>
                                            <Link to={'/'} className='button border_btn'>
                                                <span>Mehr erfahren
                                                    <svg width="40" height="15" viewBox="0 0 40 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        <path d="M39.9424 7.06055L32.4659 14.1211L30.9535 12.5198L36.7333 7.06055L30.9535 1.60128L32.4659 0L39.9424 7.06055Z" fill="inherit" />
                                                        <path d="M38.3375 5.95908V8.16201H0V5.95908H38.3375Z" fill="inherit" />
                                                    </svg></span>
                                            </Link>
                                        </div>
                                    </div>
                                </div>
                                <div className='col-md-6 col_item' data-aos='fade-right'>
                                    <div className='col_inner'>
                                        <h3>Grundrisse / Isometrie</h3>
                                        <p>Wir haben höchste Ansprüche und es soll kinderleicht zu bedienen sein. Wir geben Ihrer Webseite ein performantes Zuhause.</p>
                                        <div className='btn_block'>
                                            <Link to={'/'} className='button border_btn'>
                                                <span>Mehr erfahren
                                                    <svg width="40" height="15" viewBox="0 0 40 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        <path d="M39.9424 7.06055L32.4659 14.1211L30.9535 12.5198L36.7333 7.06055L30.9535 1.60128L32.4659 0L39.9424 7.06055Z" fill="inherit" />
                                                        <path d="M38.3375 5.95908V8.16201H0V5.95908H38.3375Z" fill="inherit" />
                                                    </svg></span>
                                            </Link>
                                        </div>
                                    </div>
                                </div>
                                <div className='col-md-6 col_item' data-aos='fade-left'>
                                    <div className='col_inner'>
                                        <h3>Social Media</h3>
                                        <p>Wir leiten Schulungen für Unternehmen, Teams und Einzelpersonen zu diversen digitalen Themen. Wir freuen uns, Sie in die digitale Welt mitzunehmen.</p>
                                        <div className='btn_block'>
                                            <Link to={'/'} className='button border_btn'>
                                                <span>Mehr erfahren
                                                    <svg width="40" height="15" viewBox="0 0 40 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        <path d="M39.9424 7.06055L32.4659 14.1211L30.9535 12.5198L36.7333 7.06055L30.9535 1.60128L32.4659 0L39.9424 7.06055Z" fill="inherit" />
                                                        <path d="M38.3375 5.95908V8.16201H0V5.95908H38.3375Z" fill="inherit" />
                                                    </svg></span>
                                            </Link>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </section>
        </div>
    )
}

export default Angebot