...
const Referenzen = ({ references }) => {
  const data = references || [];

  const options = {
    loop: data.length > 2,
    margin: 20,
    autoWidth: data.length > 2,
    dots: false,
    nav: data.length > 2,
    center: data.length === 2 ? false : true, // center for 1 or 2 items
    autoplay: false,
    smartSpeed: 600,
    responsive: {
      0: { items: 1, autoWidth: false },
      768: { items: data.length === 2 ? 2 : 1 },
      1200: { items: data.length > 2 ? 3 : data.length },
    },
    navText: [
...
//     margin: 20,
//     autoWidth: data.length > 2, // only use autoWidth when more than 2
//     dots: false,
...
//                         <Link to={'/'}>
//                             <div className='item_img'>
//                                 <img src={`https://backend.loewenmut.ch${item?.Bild?.url}`} alt='#' />
//                             </div>
//                             <div className='item_text'>
//                                 <h3>{item?.Titel}</h3>
//                                 <p>{item?.Beschreibung}</p>
